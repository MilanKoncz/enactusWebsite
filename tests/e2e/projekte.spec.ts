...
test.describe("/projekte", () => {
  ...
  test("expanding a project card reveals its detail without a page navigation", async ({ page }) => {
    await page.goto("/projekte");
    // The expand/collapse label is the toggle's own sr-only suffix
    // (ProjectsActive.tsx) — needed to disambiguate from the photo
    // lightbox triggers, which are also named "SmileGreen …" now
    // (ImageLightbox's "<alt> vergrößern" label) and also carry
    // aria-expanded, since a Radix Dialog.Trigger sets that too.
    const button = page.getByRole("button", { name: /SmileGreen (Mehr erfahren|Weniger anzeigen)/ });
  ...
  test("opening a second card closes the first, and the page never scrolls sideways mid-animation", async ({
    ...
    await page.goto("/projekte");
    const first = page.getByRole("button", { name: /SmileGreen (Mehr erfahren|Weniger anzeigen)/ });
    const second = page.getByRole("button", { name: /Mealyo (Mehr erfahren|Weniger anzeigen)/ });
