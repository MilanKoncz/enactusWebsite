...
import { faqEntries } from "@/content/innolab";
import { board } from "@/content/board";

type FaqCopyKey = Parameters<ReturnType<typeof useTranslations<"InnoLabPage.faq">>>[0];

// Same direct contact as /ideathon's own FAQ (IdeathonFaq.tsx) — Philip
// Strobl (Inno-Lead) is the board's named contact for InnoLab questions too,
// per the draft's own contact card.
const INNOLAB_CONTACT_SLUG = "philip-strobl";

export function InnoLabFaq() {
  const t = useTranslations("InnoLabPage.faq");
  const tContact = useTranslations("InnoLabPage.contact");
  const contact = board.find((member) => member.slug === INNOLAB_CONTACT_SLUG) ?? null;
