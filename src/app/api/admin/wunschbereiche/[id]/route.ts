...
import { deleteProjectArea, updateProjectArea } from "@/lib/db";
...
export async function PATCH(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    const updated = await updateProjectArea(id.data, parsed.data);
    if (!updated) {
...
export async function DELETE(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    const deleted = await deleteProjectArea(id.data);
    if (!deleted) {
      return NextResponse.json({ ok: false, error: "not_found" }, { status: 404 });
    }
    revalidateTag(PROJECT_AREAS_TAG, PROJECT_AREAS_REVALIDATE);
    ...
    console.error("Failed to delete project area", error);
    return NextResponse.json({ ok: false, error: "server_error" }, { status: 500 });
  }
}
